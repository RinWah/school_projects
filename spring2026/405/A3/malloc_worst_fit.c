...
struct block_meta *find_worst_fit(struct block_meta **last, size_t size) {
...
void *ts_malloc(size_t size) {
    ...
    if(!global_base){
        ...
        block = find_worst_fit(&last, size);
        if(!block){
...
void print_leak_report(int tid) {
    ...
    printf("\n--- Thread %d Worst Fit Leak Report ---\n", tid);
    while (current) {
        overhead += META_SIZE;
        if (!current->free) total_leaks += current->size;
        current = current->next;
    }
...
void* thread_routine(void* arg) {
    ...
    // Reallocate and free some, but leave others (to create "leaks")
    for(int i = 0; i < 10; i++) {
    ...
    // Report what's left in this thread's heap
